...
export class BlockedUserController {
  ...
  @ApiResponse({ status: 200, description: 'Successful retrieval of blocked list', type: PrismaModel.BlockedUser, isArray: true })
  @ApiResponse({ status: 401, description: 'Unauthorized, access token is invalid' })
  @ApiResponse({ status: 404, description: "User couldn't be indentified with the userId" })
