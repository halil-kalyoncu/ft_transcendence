...
export class DirectMessageController {
  ...
  @ApiResponse({ status: 200, description: 'Successful retrieval of direct messages', type: PrismaModel.DirectMessage, isArray: true })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: "Return the number of messages that a user hasn't read yet from other users" })
  @ApiResponse({ status: 200, description: 'Successful retrieval of the unread messages amount', type: UnreadMessagesDto, isArray: true })
  @ApiResponse({ status: 500, description: 'Server error' })
  @ApiBearerAuth('access-token')
  @Get('allUnreadByUserId')
