...
const setNewChannelMessages = async () => {
  try {
    const response = await fetch(
      `http://l${import.meta.env.VITE_IPADDRESS}:${import.meta.env.VITE_BACKENDPORT}/api/channel-message/getChannelMessagesforChannel?channelId=${channelId}&userId=${userId.value}`,
      {
        method: 'GET',
        headers: {
...
const updateMutedUsers = async () => {
  try {
    const response = await fetch(
      `http://${import.meta.env.VITE_IPADDRESS}:${import.meta.env.VITE_BACKENDPORT}/api/channel/updateMutedUsers?channelId=${channelId}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
