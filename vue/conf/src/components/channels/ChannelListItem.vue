...
const comparePassword = async (): Promise<boolean> => {
  try {
    const response = await fetch(
      `http://${import.meta.env.VITE_IPADDRESS}:${import.meta.env.VITE_BACKENDPORT}/api/channel/comparePassword?channelId=${props.channelId}&password=${password.value}`,
      {
        method: 'GET',
        headers: {
...
const setUserBanned = async () => {
  try {
    const response = await fetch(
      `http://${import.meta.env.VITE_IPADDRESS}:${import.meta.env.VITE_BACKENDPORT}/api/channel/isUserBanned?channelId=${props.channelId}&userId=${userId.value}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
