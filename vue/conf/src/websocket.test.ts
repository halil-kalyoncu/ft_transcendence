...
async function getUnreadMessages(userId: number) {
    const response = await fetch(`http://localhost:3000/api/directMessages/allUnreadByUserId?userId=${userId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
...
async function startSocketConnection() {
    try {
        ...
        socket_hkalyonc.on('newDirectMessage', (responseData: directMessageI) => {
            ...
            console.log(responseData);
            getUnreadMessages(userId_hkalyonc)
                .then(unreadMessages => {
        ...
        socket_mschlenz.on('newDirectMessage', (responseData: directMessageI) => {
            ...
            console.log(responseData);
            getUnreadMessages(userId_mschlenz)
                .then(unreadMessages => {
        ...
        socket_hkalyonc.emit('sendDirectMessage', {senderId: userId_hkalyonc, receiverId: userId_mschlenz, message: '1'});
        socket_elenz.emit('sendDirectMessage', {senderId: userId_elenz, receiverId: userId_mschlenz, message: '2'});
